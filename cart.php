<?php
	...
	if (isset($_GET['action']) && $_GET['action']=="add")
	{
		...
		{	
			// информация о пицце
			$sql = 'SELECT view_flower, view_price FROM flower_view WHERE view_flower = :view_id';
			...
			if ($view)
			{
				...
				$get_order = $db->prepare('SELECT * FROM flower_cart WHERE session_id = :sess_id AND acc_id = :acc_id AND view_flower = :view_flower');
			
				$get_order->execute([ ':sess_id' => $_COOKIE['SESSID'], ':acc_id' => $acc->acc_id, ':view_flower' => $view->view_flower ]);
				...
				if ($order)
				{
					...
					$add_cart_sql = 'UPDATE flower_cart SET count = :new_count
									 WHERE acc_id = :acc_id AND view_flower = :view_flower';

					$add_cart_params = [ 
										 ':new_count' => $order->count + 1, 
										 ':acc_id' => $acc->acc_id, 
										 ':view_flower' => $view->view_flower
									   ];
				}
				else
				{
					$add_cart_sql = 'INSERT INTO flower_cart (session_id, price, count, view_flower, acc_id) 
								 	 VALUES (:sess_id, :price, :count, :view_flower, :acc_id)';
	...
	{
		$sql = 'DELETE FROM flower_cart WHERE session_id = :sess_id AND view_flower = :view_id';
				...
				<div class="wrapper">
					...
					<form method="post" action="cart.php?page=cart"> 
						<table> 
							...
							?>
								<tr> 
        							<td><?php echo $row['view_flower'] ?></td> 
        						    <td><?php echo $row['count'] ?></td>
        							<td><?php echo $row['price'] ?> руб.</td> 
        							<td><?php echo $row['price'] * $row['count'] ?> руб.</td> 
        							<td> <a class="button" 
        								    href="cart.php?page=cart&action=drop&id=<?php echo $row['view_flower'] ?>">
        									Убрать
        								 </a>  
        							</td>
        						</tr>
